function [v_scores, CV_eff, CV_input, ax_CV, ax_FF, a, b, FF_gene, mu_gene] = getSingleCell_Vscores(Xnrm, Xtot, varargin)
% [v_scores CV_eff CV_input] = get_single_cell_Vstatistic(Xnrm, Xtot, varargin)
% 
%  Code to estimate the v-statistic of genes from single cell RNA sequencing data, following the Theory Supplement (Klein et al. Cell'15).
%
% BACKGROUND:
%  The v-statistic for each gene is a corrected Fano Factor accounting for noise in method efficiency, and variation in cell size. Genes with highest
% v-scores are the most biologically variable. (The Fano Factor fails because high Fano Factor could result from variation in cell size or
% technical noise, particularly for very highly expressed genes). The v-stastistic is defined as:
%
%                       V = F / (A + B*mu)
%
% where mu is the mean expression of UMI-filtered data after total-count normalization; 
%                      F = Var/mu                    is the Fano Factor; 
%                      A = 1 + (CV_eff)^2; 
%                      B = [CV_(1/N)]^2. 
% CV_eff is the noise in the efficiency of transcript capture between single cells, and CV_(1/N) is the variation in total
% number of poly-A target mRNA molecules per cell in the sample. See Eq. (S13) in (Klein et al., Cell 2015)
%
% This subroutine estimates CV_(1/N) from the data. 
% CV_(eff) is either independently estimated from the data, or else it is calculated from the relationship: 
%           1+[CV_(eff)]^2 = (1+CV_M^2)(1+[CV_(1/N)]^2), 
% where CV_M is the CV of the total number of counts detected per cell across all genes. 
% CV_M is directly calculated from the data. If the user chooses to estimate CV_eff directly from the data, it is done as follows: the
% algorithm searches for a value of CV_eff such that the "baseline" V-score will be as close as possible to 1. This is done by looking for the
% maximum in the histogram of the log(Fano Factor), and setting (1+CV_eff^2) = F_max, where F_max is the mode of the log(Fano Factor)
% distribution.
%
% The default is to estimate CV_(1/N) and CV_eff, but an option allows fitting CV_(1/N) only and inferring CV_eff from CV_M.
%
...
% estimation is carried out. Several options allowing tweaking the fitting algorithm.
% 
% INPUTS:
% Xnrm  : Matrix of total-count normalized single cell gene expression data with size p x q, where p=(# genes),
%                     q=(# cells).
% 
% Xtot  : Vector length q with the total number of UMI-filtered mapped transcripts per cell across all genes, before normalization.
%
% varargin       : See below for optional name/value pairs to tweak estimation of CV_(1/N) and resulting v-scores, and
...
% 'fit_CVeff'  : Logical.    Default = false.
%                          If true, CVeff is treated as an independent  fitting parameter. 
%                          If false, only CV_(1/N) is fitted with  CV_eff^2 = (1+(CV_M)^2)(1+(CV_(1/N))^2)-1.
%
% 'min_mean'    : Scalar z, double. Only include genes with average expression greater than z. Default z=0.
%
% 'fit_percentile'  : Scalar f, double. Fit baseline curve (V=1) to this specified running percentile value as function of
%                     mean. Default is f=0.1
%
% 'error_wt'      : Scalar wt. The exponent of fitting function. A value of wt=2 is least-squares (L2). wt=1 is robust (L1).
%                     Default wt=1.
%
% 'show_plot'       : Optionally plot CV vs mean with genes color-coded by  v-score.
%
% 'saturation_v'    : If plotting, this sets the value of the v-score at  which the color bar saturates. Default = 10. Set to
...
sizeMarker = 20;  % size of a marker for plotting 
...
% Plot:
p = scatter(mu_gene, CV_gene, sizeMarker, plt_v); % noLegend(p);
colormap(hsv(100))   
set(gca, 'yscale','linear', 'xscale','log');  %set(gca,'yscale','log','xscale','log');
axis square
cbar_hndl = colorbar;
